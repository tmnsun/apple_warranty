...
describe AppleWarranty::Scraper do
  ...
  context 'data fetched without warranty' do
    before(:all) do
      response_data = File.read(File.dirname(__FILE__) + '/fixtures/response_without_warranty.html')
      stub_request(:get, /013896000639712/).to_return(body: response_data)

      @scraper = AppleWarranty::Scraper.new
      @scraper.get_data('013896000639712')
    end
  ...
  context 'data fetched with expired warranty' do
    before(:all) do
      response_data = File.read(File.dirname(__FILE__) + '/fixtures/response_with_expired_warranty.html')
      stub_request(:get, /013977000323878/).to_return(body: response_data)

      @scraper = AppleWarranty::Scraper.new
      @scraper.get_data('013977000323878')
    end
    ...
    describe '.warranty_expired_at' do
      it { expect(@scraper.warranty_expired_at).to eq Date.new(2015, 1, 1) }
    end
  end
end
